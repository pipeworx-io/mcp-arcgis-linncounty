...
/**
 * Linn County GIS — Linn County, Iowa open geospatial data (ArcGIS).
 *
 * Linn County GIS (ArcGIS org i14SLLmXo7Hn9vNc): parcels, addresses, zoning & public works. search_datasets finds layers (each
...
const UA = 'pipeworx-mcp-arcgis-linncounty/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Linn County GIS open geospatial datasets (parcels, addresses, zoning & public works) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (Linn County GIS).' },
